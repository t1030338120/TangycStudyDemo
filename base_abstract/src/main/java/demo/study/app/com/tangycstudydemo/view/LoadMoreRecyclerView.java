...
/**
 *  描述：加载更多的RecyclerView，带有footer加载条。
 *  过设置OnScrollListener来实现加载更多的功能，巧妙通过一个中间的Adapter来实现添加footer的功能。
 */
public class LoadMoreRecyclerView extends RecyclerView {
  ...
  private void init() {
    addOnScrollListener(new OnScrollListener() {
      ...
      @Override public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        if (null != mListener && mIsFooterEnable && !mIsLoadingMore && dy > 0) { //dy>0：上拉动作
  ...
  /**
   * 数据已经加载完毕时 ( 没有更多数据 ) 调用，去除footer和加载更多功能。
   * @param hasMore
   */
